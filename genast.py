...
writeline(headerfile, "")
writeline(headerfile, "  void pushLevel(std::string);")
writeline(headerfile, "  void addElement(std::string);")
writeline(headerfile, "  void popLevel();")
writeline(headerfile, "")
writeline(headerfile, "public:")
for node in nodes:
...
writeline(codefile, "void Print::pushLevel(std::string name) {")
...
writeline(codefile, "void Print::popLevel() {")
writeline(codefile, "  std::stringstream ss;")
...
for node in nodes:
    ...
    if (len(node.children) > 0):
        writeline(codefile, "  this->pushLevel(\"" + node.name + "\");")
        writeline(codefile, "  node->visit_children(this);")
        writeline(codefile, "  this->popLevel();")
    else:
